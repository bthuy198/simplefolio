...
public class CustomerServlet extends HttpServlet {
    ...
    private void showCustomers(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        RequestDispatcher requestDispatcher = request.getRequestDispatcher("customer/customers.jsp");
        requestDispatcher.forward(request, response);
    }
    ...
    private void handleEditImageUpload(HttpServletRequest req, Customer customer, List<String> errors) throws ServletException, IOException {
        for (Part part : req.getParts()) {
            ...
            if(!fileName.equals("")){
                Customer customerDB = iCustomerService.findCustomerById(customer.getId());
                if (!customerDB.getImg().equals(fileName)) {
                    String appRealPath = getServletContext().getRealPath("/") + "images";
                    File file = new File(appRealPath);
                    if (!file.exists()) {
                        file.mkdir();
                    ...
                    // lay lai anh cu
                    customer.setImg(customerDB.getImg());
                }
            }
        }
    }
    ...
    private void deleteCustomer(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        response.sendRedirect("customer?delete=success");
